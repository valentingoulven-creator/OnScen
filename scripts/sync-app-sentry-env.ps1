# scripts/sync-app-sentry-env.ps1 - Genere app/.env.production depuis backend/.env.production (VITE_SENTRY_*)
param(
...
$backendEnv = Join-Path $root "backend\.env.$Environment"
$appEnv = Join-Path $root "app\.env.$Environment"
$appExample = Join-Path $root "app\.env.$Environment.example"

if (-not (Test-Path $backendEnv)) {
    throw "Missing file: $backendEnv (run: scp soundy-prod:/opt/soundly/.env backend/.env.production)"
}

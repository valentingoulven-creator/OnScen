# scripts/sync-external-env.ps1 - Sync missing external env keys msdev/local -> VPS
# Usage:
#   powershell -ExecutionPolicy Bypass -File scripts/sync-external-env.ps1 -Staging
#   powershell -ExecutionPolicy Bypass -File scripts/sync-external-env.ps1 -Prod
#   powershell -ExecutionPolicy Bypass -File scripts/sync-external-env.ps1 -Staging -Prod
#   powershell -ExecutionPolicy Bypass -File scripts/sync-external-env.ps1 -DryRun
param(
    [switch]$Staging,
    [switch]$Prod,
    [switch]$DryRun
...
$msdevEnv = Read-EnvFile (Join-Path $root 'msdev\.env')
$localProdEnv = Read-EnvFile (Join-Path $root 'backend\.env.production')
...
function Build-Entries([hashtable]$remoteMap, [string]$targetLabel) {
    ...
    foreach ($key in $fromMsdev) {
        ...
        if (-not $val) {
            Write-Host "  [skip] $key missing in msdev/.env" -ForegroundColor DarkGray
            continue
        }

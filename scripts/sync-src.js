 ...
 * scripts/sync-src.js
 * ===================
 * Outil de diagnostic et de gestion de la synchronisation app/src ↔ apptel/src.
 *
 * ARCHITECTURE
 * ------------
 * • app/src/    = SOURCE DE VÉRITÉ (webapp + logique partagée)
 * • apptel/src/ = OVERRIDES SEULEMENT (23 fichiers spécifiques téléphone)
 *
 * Le plugin Vite `apptelSrcFallback` dans apptel/vite.config.ts charge
 * automatiquement depuis app/src/ tout fichier absent de apptel/src/.
 * Le tsconfig rootDirs fait de même pour TypeScript.
 * → Aucune copie manuelle n'est nécessaire au quotidien.
 *
 * USAGE
 * -----
 *   node scripts/sync-src.js              # Alias pour --status
 *   node scripts/sync-src.js --status     # Afficher l'état des deux src/
 *   node scripts/sync-src.js --check      # Vérifier qu'il n'y a pas de doublons
 *   node scripts/sync-src.js --clean      # Supprimer les doublons identiques
 *   node scripts/sync-src.js --clean --dry-run  # Voir ce qui serait supprimé
 *
 * FICHIERS PROTÉGÉS
 * -----------------
 * Ces fichiers ont une version intentionnellement différente dans apptel/src/.
...
const TEL_SRC = join(ROOT, 'apptel', 'src');

/**
 * Fichiers spécifiques à apptel — présents dans apptel/src/ intentionnellement.
...
function runStatus() {
  console.log('\n📊  ' + C + 'Statut apptel/src vs app/src' + R + '\n');
  ...
  console.log(C + '── Overrides téléphone (apptel/src a sa propre version) ──' + R);
  ...
  if (dupList.length > 0) {
    console.log('\n' + E + '── ⚠ Doublons détectés dans apptel/src (inutiles) ──' + R);
  ...
  console.log('\n' + R + 'Résumé :');
  console.log('  ' + overrides + ' overrides téléphone dans apptel/src/');
...
function runCheck() {
  ...
  if (issues === 0) {
    ...
    console.log('\n' + E + issues + ' doublon(s) détecté(s). Lancer: node scripts/sync-src.js --clean' + R);
    process.exit(1);
  }
}
...
function runClean() {
  ...
  console.log('\n🧹  ' + mode + 'Nettoyage des doublons dans apptel/src/\n');

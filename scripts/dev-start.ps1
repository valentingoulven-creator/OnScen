# scripts/dev-start.ps1 — Lance l'environnement DEV local (pas la prod)
# Backend msdev :4080 + frontend Vite :5173
# Usage : npm run dev   ou   powershell -ExecutionPolicy Bypass -File scripts/dev-start.ps1
param(
    [switch]$NoBrowser
)
...
if (-not (Test-Path $msdevEnv)) {
    ...
    if (Test-Path $example) {
        Write-Host '[!] msdev/.env absent — copiez msdev/.env.example vers msdev/.env' -ForegroundColor Yellow
    }
}

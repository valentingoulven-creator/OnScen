 ...
 *   node scripts/update-well-known-mobile.mjs --from-keystore
 */
...
const envFile = path.join(root, 'mobile-store.env');
...
function sha256FromKeystore() {
  const propsPath = path.join(root, 'apptel/android/keystore.properties');
...
function main() {
  ...
  writeJson('app/public/.well-known/assetlinks.json', assetlinks);
  writeJson('backend/public/.well-known/assetlinks.json', assetlinks);
  ...
  if (teamId) {
    ...
    writeJson('backend/public/.well-known/apple-app-site-association', aasa);
  } else {

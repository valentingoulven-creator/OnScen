...
# Voir scripts/SETUP-SECOND-CURSOR.md et scripts/secrets-checklist.template.txt
...
function Write-AppEnvDevelopmentIfMissing([string]$Root) {
    $dest = Join-Path $Root 'app\.env.development'
    if (Test-Path $dest) {
        Write-Ok 'app/.env.development deja present'
        return
    }
    @(
        '# Dev local (Vite npm run dev / scripts/dev-start.ps1)',
        '# Pas de secrets ici - msdev/.env reste local et gitignore',
        'VITE_APP_ENV=msdev',
        'VITE_WEB_APP_URL=http://localhost:5173'
    ) | Set-Content -Path $dest -Encoding UTF8
    Write-Ok 'app/.env.development cree (valeurs dev par defaut)'
}
...
if (Copy-EnvIfMissing -Root $repoRoot -DestRelative 'msdev\.env' -ExampleRelative 'msdev\.env.example' -Label 'msdev/.env') {
    $createdEnvFiles += 'msdev/.env'
}
if (Copy-EnvIfMissing -Root $repoRoot -DestRelative 'backend\.env.production' -ExampleRelative 'backend\.env.production.example' -Label 'backend/.env.production') {
    $createdEnvFiles += 'backend/.env.production'
}
...
Write-Host '     scripts/SETUP-SECOND-CURSOR.md'
Write-Host '     scripts/secrets-checklist.template.txt'
Write-Host ''

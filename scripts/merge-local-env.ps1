# merge-local-env.ps1 — Copie variables manquantes entre msdev/.env et backend/.env.production
param(
    [switch]$DryRun,
    [switch]$FromProd   # backend/.env.production -> msdev/.env (defaut: msdev -> backend)
)

$ErrorActionPreference = 'Stop'
$root = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$msdevPath = Join-Path $root 'msdev\.env'
$prodPath = Join-Path $root 'backend\.env.production'
...
if (-not (Test-Path $msdevPath)) { throw "msdev/.env introuvable" }
if (-not (Test-Path $prodPath)) {
    Copy-Item (Join-Path $root 'backend\.env.production.example') $prodPath
}

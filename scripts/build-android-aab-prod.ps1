...
# Prérequis : JDK 21, Android SDK, keystore signé (voir apptel/android/keystore.properties.example)
...
$keystoreProps = Join-Path $root "apptel\android\keystore.properties"
if (-not (Test-Path $keystoreProps)) {
  Write-Error "Fichier keystore manquant : apptel/android/keystore.properties (copier depuis keystore.properties.example)"
}
...
Push-Location $root
try {
  npm run build:capacitor:prod --prefix apptel
  npm run cap:sync:android --prefix apptel
  Push-Location (Join-Path $root "apptel\android")
  try {
    .\gradlew.bat bundleRelease
  } finally {
    Pop-Location
  }
  $aabSrc = Join-Path $root "apptel\android\app\build\outputs\bundle\release\app-release.aab"
  $aabDest = Join-Path $root "MeloSong-Mobile\Soundy-release-prod.aab"
  if (Test-Path $aabSrc) {

  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        {story.imageUrl ? (
          <img src={story.imageUrl} alt="" className="w-full h-full object-cover" draggable={false} />
        ) : (
          <div className="w-full h-full bg-gradient-to-b from-[#1a1028] via-[#0b0b0f] to-[#12121a]" />
        )}
        <div className="absolute inset-0 bg-gradient-to-b from-black/55 via-transparent to-black/70 pointer-events-none" />
      </div>

      {/* Barres de progression */}
      <div className="absolute top-0 inset-x-0 z-20 flex gap-1 px-2 pt-[max(0.5rem,env(safe-area-inset-top))]">
        {stack.map((seg, i) => {
          let fill = 0;
          if (i < stackIndex) fill = 1;
          else if (i === stackIndex) fill = progress;
          return (
            <div key={seg.id} className="flex-1 h-[3px] rounded-full bg-white/25 overflow-hidden">
              <div
      ...
      {/* En-tête */}
      <div className="absolute top-0 inset-x-0 z-20 flex items-center justify-between gap-2 px-3 pt-8 pb-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <UsernameDisplay
              ...
              className="text-sm font-semibold truncate block text-white drop-shadow"
            />
            <p className="text-[11px] text-white/70 drop-shadow">{formatStoryTimeAgo(story.createdAt)}</p>
          </div>
          {story.visibility ? (
        ...
        <button
          type="button"
          onClick={onClose}
          className="p-2 rounded-full text-white/90 hover:bg-white/10 shrink-0"
          aria-label="Fermer"
        >
      ...
      <div className="absolute inset-x-0 bottom-0 z-20 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] space-y-2 pointer-events-none">
        {story.content ? (
          <p className="text-sm text-white whitespace-pre-wrap break-words drop-shadow-lg">{story.content}</p>
        ) : null}

        {story.musicTrack ? (
          <div className="flex items-center gap-2 rounded-xl bg-black/50 border border-white/10 px-3 py-2 pointer-events-auto max-w-sm">
            ...
            <div className="min-w-0 flex-1">
              ...
              {story.musicTrack.artist ? (
                <p className="text-[10px] text-white/60 truncate">{story.musicTrack.artist}</p>
              ) : null}
            </div>
        ...
        {story.taggedUsers && story.taggedUsers.length > 0 ? (
          <div className="flex flex-wrap gap-1.5">
            {story.taggedUsers.map((t) => (
              <span
                key={t.id}
                className="inline-flex items-center gap-1 rounded-full bg-black/50 border border-white/15 pl-1 pr-2 py-0.5"
              >

...
interface SalonParticipantsPopoverProps {
  salonId: string;
  token: string;
  onVipChange?: (userId: string, isVip: boolean) => void;
...
export function SalonParticipantsPopover({
  ...
  const toggleVip = (p: SalonParticipant) => {
    const add = !p.isVip;
    if (!add) {
    ...
    emitOnSocket('salon_set_vip', { salonId, userId: p.id, add });
    setParticipants((prev) =>
      prev.map((x) => (x.id === p.id ? { ...x, isVip: add } : x))
    );
    onVipChange?.(p.id, add);
    onActionDone?.(
      add
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1">
                    <button
                      type="button"
                      onClick={() => toggleVip(p)}
                      className="px-2 py-0.5 rounded-md text-[10px] font-semibold border border-amber-500/30 text-amber-200 hover:bg-amber-950/30"
                    >
                      {p.isVip

...
import { ACCEPTED_IMAGE_FORMATS, validateProfilePhoto } from '../lib/imageConstraints';
...
export function ProfilePhotoGallery({
  ...
  const showingPlaceholder = photos.length === 0;
        ...
        <div className="bg-[#12121a] border border-[#1e1e2f] rounded-2xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            {photos.length < MAX_PHOTOS && (
              ...
              >
                <span className="text-2xl">{editorFile ? '…' : '🖼️'}</span>
          ...
          {photos.length < MAX_PHOTOS && (
            <>
              ...
              >
                ...
                {editorFile ? 'Modifiez votre photo…' : 'Ajouter une photo (galerie)'}
              </button>
              {validationError ? (
                <p className="text-[10px] text-red-400 text-center">{validationError}</p>
              ) : (
                <p className="text-[10px] text-gray-600 text-center">
                  JPEG, PNG, WebP · max 8 Mo · rognage et filtres · compression auto
                </p>
              )}
    ...
    <div className="space-y-2">
      ...
      {list.length > 1 && (
        <p className="text-center text-[10px] text-gray-500">
          ...
          {!showingPlaceholder ? ' · la première photo est l’avatar' : ''}
        </p>
      )}
      {photos.length > 0 && list.length === 1 && (
        <p className="text-center text-[10px] text-gray-500">1 photo</p>
      )}
    </div>
  );
}

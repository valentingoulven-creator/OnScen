...
scripts/setup-staging-infra.ps1      # bootstrap VPS + base PG + SSH config
scripts/setup-staging-env.ps1        # génère .env preprod + push VPS
...
- `deploy/environments.ps1` — config prod / preprod
- `deploy_zero_downtime.ps1 -Environment preprod|prod`
- `deploy/Caddyfile.staging` — HTTPS staging + Basic Auth équipe
- `deploy/ecosystem.staging.config.cjs` — PM2 `melosong-backend-staging`

Voir aussi : `docs/INFRA-SOUNDY.md`, `deploy/RUNBOOK-PROD.md`.

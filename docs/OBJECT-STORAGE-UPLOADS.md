...
État actuel : les uploads (avatars, reels, sponsors, pièces jointes chat) sont stockés **sur disque VPS** (`backend/public/uploads/`). Les backups off-site peuvent sync vers un bucket S3 Scaleway (`deploy/backup-offsite.sh`).
...
Voir aussi : `deploy/setup-scaleway-object-storage.sh`, `docs/INFRA-SOUNDY.md`.

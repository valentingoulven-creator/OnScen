...
# Usage (root sur VPS) : bash /opt/soundy/deploy/setup-phase0-prod.sh
set -euo pipefail
...
main() {
  ...
  configure_s3_env
  log "Terminé. Relancer PM2 : pm2 startOrReload /opt/soundy/deploy/ecosystem.config.cjs --update-env"
}

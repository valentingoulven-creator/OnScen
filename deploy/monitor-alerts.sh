#!/usr/bin/env bash
# monitor-alerts.sh — Alertes système VPS Soundy
...
#
# Config :
#   RESEND_API_KEY (prioritaire, HTTP 443) ou credentials SMTP depuis /opt/soundy/.env.
#   Les seuils peuvent être surchargés via des variables d'environnement.
#
# Usage manuel :
#   sudo bash /opt/soundy/deploy/monitor-alerts.sh
#
...
#   Manuel : bash /opt/soundy/deploy/pm2-reload-intentional.sh [reload|restart]
set -euo pipefail
...
PREV_RESTART_FILE="/tmp/soundy_pm2_restarts"
...
is_coolingdown() {
  local name="$1"
  local lockfile="/tmp/soundy_alert_${name}"
  if [[ -f "$lockfile" ]]; then
...
send_alert_email() {
  ...
  if [[ "$RESEND_ENABLED" == "true" ]]; then
    local json_payload http_code response_file="/tmp/soundy_resend_response.json"
...
if [[ "$DISK_USED" -ge "$DISK_THRESHOLD" ]]; then
  if ! is_coolingdown "disk"; then
    ...
    send_alert_email \
      "💾 [Soundy ${SEVERITY}] Disque ${DISK_USED}% — seuil ${DISK_THRESHOLD}%" \
...
if [[ -n "$RAM_INFO" ]]; then
  ...
  if [[ "$RAM_PERCENT" -ge "$RAM_THRESHOLD" ]]; then
    if ! is_coolingdown "ram"; then
      ...
      send_alert_email \
        "🧠 [Soundy ${SEVERITY}] RAM ${RAM_PERCENT}% — seuil ${RAM_THRESHOLD}%" \
...
if [[ "$CPU_PERCENT" -ge "$CPU_THRESHOLD" ]]; then
  if ! is_coolingdown "cpu"; then
    ...
    send_alert_email \
      "⚙️ [Soundy ${SEVERITY}] CPU ${CPU_PERCENT}% — seuil ${CPU_THRESHOLD}%" \
  ...
  if [[ "$PM2_RESTARTS" =~ ^[0-9]+$ ]]; then
    ...
    if [[ "$PM2_RESTARTS" -gt "$PREV" && "$PREV" != "0" ]]; then
      ...
      if is_intentional_pm2_reload; then
        ...
        rm -f "/tmp/soundy_alert_pm2_crash" 2>/dev/null || true
        send_alert_email \

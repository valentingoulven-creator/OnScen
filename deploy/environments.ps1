# deploy/environments.ps1 — Cibles de deploiement Soundy (prod / preprod)
# Usage : . ./deploy/environments.ps1 ; $cfg = Get-SoundyDeployEnvironment preprod
...
function Get-SoundyDeployEnvironment {
    ...
    $map = @{
        prod = @{
            ...
            ViteMode       = 'production'
            ViteEnvFile    = 'app/.env.production'
        ...
        preprod = @{
            ...
            ViteEnvFile    = 'app/.env.preproduction'
            Caddyfile      = 'deploy/Caddyfile.staging'
            EcosystemFile  = 'deploy/ecosystem.staging.config.cjs'

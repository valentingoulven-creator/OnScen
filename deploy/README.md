...
Le fichier **`deploy/Caddyfile`** est la **seule source de vérité** (getsoundy.com + IP + Basic Auth + bypass API).
...
# ou
sudo ./deploy/postgres-setup.sh --local-apt
./deploy/postgres-setup.sh --env-snippet   # modèle .env
```

...
#
# Usage : bash /opt/soundy/deploy/verify-scaleway-backup.sh
set -euo pipefail
...
echo "  [ ] Clés API créées → SCW_ACCESS_KEY / SCW_SECRET_KEY dans /opt/soundy/.env"
echo "  [ ] SCW_BUCKET défini → backup-offsite.sh sync S3"
echo ""
echo "VPS local (automatisé via cron) :"
echo "  bash /opt/soundy/deploy/verify-prod.sh"
echo ""
echo "Pour activer les crons locaux :"
echo "  sudo bash /opt/soundy/deploy/install-backup-cron.sh"
echo "  sudo bash /opt/soundy/deploy/install-uploads-backup-cron.sh"
echo "  sudo bash /opt/soundy/deploy/install-offsite-backup-cron.sh"
echo ""

...
SYNC="$ROOT/deploy/sync-caddy.sh"
...
chmod +x "$ROOT/deploy/sync-caddy.sh"
chmod +x "$ROOT/deploy/caddy-watchdog.sh"
chmod +x "$ROOT/deploy/healthcheck.sh" 2>/dev/null || true
chmod +x "$ROOT/deploy/backup-db.sh" 2>/dev/null || true
chmod +x "$ROOT/deploy/verify-backup.sh" 2>/dev/null || true
chmod +x "$ROOT/deploy/verify-prod.sh" 2>/dev/null || true
chmod +x "$ROOT/deploy/install-backup-cron.sh" 2>/dev/null || true
chmod +x "$ROOT/deploy/install-health-cron.sh" 2>/dev/null || true
chmod +x "$ROOT/deploy/setup-legal-publisher.sh" 2>/dev/null || true
mkdir -p "$ROOT/backups" "$ROOT/logs"

cp "$ROOT/deploy/caddy-watchdog.sh" "$WATCHDOG_DST"
chmod +x "$WATCHDOG_DST"

if [ -f "$ROOT/deploy/healthcheck.sh" ]; then
  cp "$ROOT/deploy/healthcheck.sh" "$HEALTH_DST"
  chmod +x "$HEALTH_DST"
fi

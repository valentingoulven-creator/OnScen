  ...
  return {
    architecture: {
      monorepo: {
        ...
        apptel: 'Capacitor 8 overrides mobile uniquement (apptel/src/)',
        backend: 'Express TS monolithe (backend/src/) — routes /api, Socket.io',
        msdev: 'Dev local : npm run dev → :5173 + API :4080 (APP_ENV=msdev)',
        deploy: 'scripts/deploy-prod.ps1 · deploy-preprod.ps1 · PM2 VPS Scaleway',
      },
      keyDomains: [
      ...
      dataLayer: {
        ...
        target: 'PG source de vérité + Redis cache/queues + S3 uploads (docs/STACK-CIBLE.md)',
    ...
    testCommands: {
      ...
      verifyAccess: 'scripts/verify-full-access.ps1',
    },
  };
}

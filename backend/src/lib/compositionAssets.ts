...
export const COMPOSITION_AUDIO_DATA_RE =
  /^data:audio\/(mpeg|mp3|wav|x-wav|mp4|x-m4a|ogg|webm|x-flac|flac)(?:;[^;,]+)*;base64,([A-Za-z0-9+/=]+)$/i;

export const UPLOADS_FILE_RE = /^\/uploads\/compositions\/[a-zA-Z0-9._-]+\.(mp3|wav|m4a|ogg|webm|flac)$/i;
...
/** Décode une data URL audio validée (sans écrire le fichier). */
export function decodeCompositionDataUrl(dataUrl: string): { buffer: Buffer; mime: string } {
...
export function saveCompositionBuffer(buffer: Buffer, mime: string): string {
...
export function saveCompositionFromDataUrl(dataUrl: string): string {
  const { buffer, mime } = decodeCompositionDataUrl(dataUrl);
  return saveCompositionBuffer(buffer, mime);
}

...
> = [
  ...
  () => `J'adore quand on tombe sur une pépite musicale par hasard 💎 La magie de Soundly`,
  ...
  () => `Soirée entre amis, on écoute de la musique ensemble sur Soundly 🎶✨`,
  () => `Mon coup de cœur de la semaine, je vous le partage ici ❤️‍🔥`,
  () => `Music is life 🎵 — sur Soundly encore une fois, impossible de décrocher !`,
  ...
  () => `Découverte grâce à la carte — merci Soundly 🗺️❤️`,
  ...
  () => `Soundly + weekend + bonne humeur = combo parfait ☀️🎵`,
  ...
  () => `Vendredi soir = soirée musique sur MeloSong, qui est partant ? 🎉`,

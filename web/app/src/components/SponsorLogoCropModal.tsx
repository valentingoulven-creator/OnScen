...
import {
  exportSponsorLogoDataUrl,
  loadSponsorLogoBitmap,
  SPONSOR_LOGO_CROP_VIEWPORT_PX,
  validateSponsorLogoFile,
} from '../lib/sponsorLogoUpload';

interface SponsorLogoCropModalProps {
  file: File;
  onConfirm: (dataUrl: string) => void;
  onCancel: () => void;
}

export function SponsorLogoCropModal({ file, onConfirm, onCancel }: SponsorLogoCropModalProps) {
  const { t } = useTranslation();
  const viewportSize = SPONSOR_LOGO_CROP_VIEWPORT_PX;
  ...
  useEffect(() => {
    ...
    void loadSponsorLogoBitmap(file)
      .then((bmp) => {
        if (cancelled) {
          bmp.close();
          return;
        ...
        loaded = bmp;
        const base = initialCoverScale(bmp.width, bmp.height, viewportSize);
        setBitmap(bmp);
        setScale(base);
        setOffset({ x: 0, y: 0 });
      })
  ...
  const confirm = useCallback(() => {
    ...
    setExporting(true);
    try {
      const crop = computeCropRectFromViewport(
        bitmap.width,
        bitmap.height,
        viewportSize,
        scale,
      ...
      const dataUrl = exportSponsorLogoDataUrl(bitmap, crop);
      onConfirm(dataUrl);
    } catch (e) {
  ...
  }, [bitmap, scale, offset, onConfirm, viewportSize]);

  const minScale = bitmap ? initialCoverScale(bitmap.width, bitmap.height, viewportSize) : 1;
  ...
  return (
    ...
    >
      <div className="w-full max-w-sm bg-[#12121a] border border-[#2d2d3d] rounded-2xl p-4 space-y-4 shadow-2xl">
        <h2 id="sponsor-logo-crop-title" className="text-lg font-bold text-white text-center">
          {t('admin.sponsors.logoCropTitle')}
        </h2>
        <p className="text-xs text-gray-400 text-center">{t('admin.sponsors.logoCropHint')}</p>
        ...
        {!loading && bitmap && (
          <>
              ...
              {previewUrl && (
                <img
                  ...
                  style={{
                    ...
                    left: (viewportSize - bitmap.width * scale) / 2 + offset.x,
                    top: (viewportSize - bitmap.height * scale) / 2 + offset.y,
                  }}
                />
              )}
              <div
            ...
            <label className="block">
              <span className="text-xs text-gray-400">{t('admin.sponsors.logoCropZoom')}</span>
              <input
                type="range"
                min={minScale}
                max={maxScale}
        ...
        <div className="flex gap-2">
          ...
          >
            {t('admin.sponsors.logoCropCancel')}
          </button>
          <button
            type="button"
            onClick={confirm}
          ...
          >
            {exporting ? '…' : t('admin.sponsors.logoCropConfirm')}
          </button>
        </div>
      </div>
    </div>

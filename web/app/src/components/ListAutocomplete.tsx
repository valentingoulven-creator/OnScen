...
import type { ListSuggestion } from '../lib/listSearch';
...
interface ListAutocompleteProps {
  ...
  exclude?: string[];
  searchFn: (query: string, exclude: string[]) => ListSuggestion[];
  placeholder?: string;
  className?: string;
...
export function ListAutocomplete({
  ...
  emptyHint = 'Aucune suggestion — validez votre saisie',
  onKeyDown,
  ...
  const suggestions = useMemo(() => searchFn(value, exclude), [value, exclude, searchFn]);
  ...
  const pick = (suggestion: ListSuggestion) => {

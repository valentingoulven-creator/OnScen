...
function applyLegalPublisherEnvOverrides(config: LegalPublisherConfig): LegalPublisherConfig {
  const overrides: Partial<Record<keyof LegalPublisherConfig, string | undefined>> = {
    publisherName: process.env.LEGAL_PUBLISHER_NAME,
    legalForm: process.env.LEGAL_PUBLISHER_LEGAL_FORM,
    address: process.env.LEGAL_PUBLISHER_ADDRESS,
    siren: process.env.LEGAL_PUBLISHER_SIREN,
    rcs: process.env.LEGAL_PUBLISHER_RCS,
    capital: process.env.LEGAL_PUBLISHER_CAPITAL,
    publicationDirector: process.env.LEGAL_PUBLISHER_DIRECTOR,
    hostName: process.env.LEGAL_PUBLISHER_HOST_NAME,
    hostAddress: process.env.LEGAL_PUBLISHER_HOST_ADDRESS,
    hostPhone: process.env.LEGAL_PUBLISHER_HOST_PHONE,
    hostCountry: process.env.LEGAL_PUBLISHER_HOST_COUNTRY,
    mediatorName: process.env.LEGAL_PUBLISHER_MEDIATOR_NAME,
    mediatorUrl: process.env.LEGAL_PUBLISHER_MEDIATOR_URL,
    dpoEmail: process.env.LEGAL_PUBLISHER_DPO_EMAIL,
    contactEmail: process.env.LEGAL_PUBLISHER_CONTACT_EMAIL,
    privacyEmail: process.env.LEGAL_PUBLISHER_PRIVACY_EMAIL,
    productionDomain: process.env.LEGAL_PUBLISHER_DOMAIN,
  };
  for (const [key, raw] of Object.entries(overrides) as [keyof LegalPublisherConfig, string | undefined][]) {
    const v = raw?.trim();
    if (v) config[key] = v;
  }
  return config;

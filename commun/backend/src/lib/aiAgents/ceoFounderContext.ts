...
export function computeCeoDataGaps(
  ...
  if (!founder) {
    gaps.push({
      ...
      severity: 'critical',
      question:
        'Pouvez-vous créer commun/msdev/ceo-founder-context.json (copie de ceo-founder-context.example.json) avec trésorerie, ville pivot et objectifs du trimestre ?',
      whyItMatters: 'Sans ce fichier, je pilote à l’aveugle sur burn, runway et priorités GTM.',
      suggestedField: 'commun/msdev/ceo-founder-context.json',
    });
    return gaps;
  }

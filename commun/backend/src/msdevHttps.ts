...
import { getAppRoot, getMsdevDir, isPackaged } from './paths';
...
function getMsdevCertDir(): string {
  ...
  return path.join(getMsdevDir(), 'certs');
}
...
export function ensureMsdevHttpsCredentials(): { key: Buffer; cert: Buffer } | null {
  ...
  if (!fs.existsSync(keyPath) || !fs.existsSync(certPath)) {
    ...
    try {
      ...
      console.error('    Installez OpenSSL ou lancez commun/msdev/scripts/generate-dev-cert.ps1');

...
export function getPool(): Pool {
  if (!pool) {
    ...
    pool.on('error', (err) => {
      console.error('[onscen] Erreur pool PostgreSQL :', err.message);
    });

    // Log au démarrage pour confirmer la config
    console.log(
      `[onscen] Pool PostgreSQL initialisé — max=${config.max} ` +
...
export async function closePool(): Promise<void> {
  ...
  try {
    ...
    console.warn('[onscen] Erreur à la fermeture du pool PostgreSQL :', err);
  }
}

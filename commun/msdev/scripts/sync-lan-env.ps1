# Met a jour commun/msdev/.env avec l'IP LAN du PC (interface Internet par defaut) + test YouTube
...
try {
    npx ts-node --transpile-only src/commun/scripts/sync-lan-env.ts
    exit $LASTEXITCODE
} finally {
    Pop-Location
}

# commun/scripts/setup-staging-env.ps1 — Genere commun/backend/.env.preproduction et pousse .env sur VPS staging
param(
    [switch]$PushOnly,
    [switch]$GenerateOnly
)
...
$appStagingEnv = Join-Path $root 'web\app\.env.preproduction'
$appExample = Join-Path $root 'web\app\.env.preproduction.example'
...
if (-not $PushOnly) {
    Write-Host '>> Generation commun/backend/.env.preproduction' -ForegroundColor Cyan

    if (-not (Test-Path $prodEnv)) {
        if (-not (Test-Path (Join-Path $root 'commun\backend\.env.preproduction.example'))) {
            throw 'Impossible de generer .env.preproduction sans commun/backend/.env.production ou .example'
        }
        Copy-Item (Join-Path $root 'commun\backend\.env.preproduction.example') $stagingEnv -Force
    } else {
...
Write-Host ''
Write-Host 'Prochaine etape : commun/scripts/deploy-preprod.ps1' -ForegroundColor Cyan

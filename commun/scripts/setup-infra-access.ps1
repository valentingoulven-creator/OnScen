...
#   powershell -ExecutionPolicy Bypass -File commun/scripts/setup-infra-access.ps1
#
# Options :
...
function Show-PublicKeyInstructions([string]$keyPath) {
    ...
    Write-Host "    powershell -ExecutionPolicy Bypass -File commun/scripts/setup-infra-access.ps1 -TestOnly"
    Write-Host ''
}

function Pull-ProdEnvFile([string]$keyPath) {
    $dest = Join-Path $RepoRoot 'commun\backend\.env.production'
    ...
    Write-Ok "Prod env synchronise -> commun/backend/.env.production ($($remoteContent.Count) lignes)"
}
...
function Sync-MsdevFromProd {
    $prodPath = Join-Path $RepoRoot 'commun\backend\.env.production'
    $msdevPath = Join-Path $RepoRoot 'commun\msdev\.env'
    if (-not (Test-Path $prodPath)) {
        Write-Warn 'commun/backend/.env.production absent - lancez -PullProdEnv d abord'
        return
    }
    if (-not (Test-Path $msdevPath)) {
        Copy-Item (Join-Path $RepoRoot 'commun\msdev\.env.example') $msdevPath
        Write-Ok 'commun/msdev/.env cree depuis .env.example'
    }
    ...
    if ($added.Count -eq 0) {
        Write-Ok 'commun/msdev/.env deja a jour (rien a synchroniser)'
        return
    }
    $newLines | Set-Content -Path $msdevPath -Encoding UTF8
    Write-Ok ("Variables ajoutees a commun/msdev/.env : " + ($added -join ', '))
    Write-Warn 'DATABASE_URL prod copiee pour tests locaux - ne jamais committer commun/msdev/.env'
}
...
if ($ImportFile) {
    ...
    Write-Ok "Import -> commun/backend/.env.production"
...
if ($SyncMsdevFromProd) {
    Write-Step 'Sync commun/msdev/.env depuis commun/backend/.env.production'
    Sync-MsdevFromProd
}
...
$prodEnvPath = Join-Path $RepoRoot 'commun\backend\.env.production'
$msdevEnvPath = Join-Path $RepoRoot 'commun\msdev\.env'
$prodMap = Read-EnvMap $prodEnvPath
$msdevMap = Read-EnvMap $msdevEnvPath

Write-Info 'commun/backend/.env.production :'
...
if (-not $sshOk) {
    ...
    Write-Host '  2. Relancer : commun/scripts/setup-infra-access.ps1 -PullProdEnv -SyncMsdevFromProd' -ForegroundColor White
} else {
    ...
    if (-not (Test-Path $prodEnvPath) -or $prodMap.Count -lt 5) {
        Write-Host '  Lancez : commun/scripts/setup-infra-access.ps1 -PullProdEnv -SyncMsdevFromProd' -ForegroundColor White
    }
}

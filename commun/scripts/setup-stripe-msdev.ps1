# Bootstrap Stripe test account for msdev: subscription prices + webhook signing secret in commun/msdev/.env
# Usage: powershell -ExecutionPolicy Bypass -File commun/scripts/setup-stripe-msdev.ps1
...
function Get-ProjectRoot {
    $dir = $PSScriptRoot
    while ($dir) {
        if (Test-Path (Join-Path $dir 'commun\msdev\.env')) { return (Resolve-Path $dir).Path }
...
if (-not $root) {
    Write-Host '[ERREUR] commun/msdev/.env introuvable.' -ForegroundColor Red
    exit 1
}

$envFile = Join-Path $root 'commun\msdev\.env'
if (-not (Test-Path $envFile)) { $envFile = Join-Path $root 'msdev\.env' }
$match = Select-String -Path $envFile -Pattern '^STRIPE_SECRET_KEY=(.+)$'
if (-not $match) {
    Write-Host '[ERREUR] STRIPE_SECRET_KEY manquant dans commun/msdev/.env' -ForegroundColor Red
    exit 1
}
...
Write-Host ''
Write-Host 'commun/msdev/.env mis a jour.' -ForegroundColor Green
Write-Host 'Lancez les webhooks locaux : commun/scripts/stripe-listen-msdev.ps1' -ForegroundColor Cyan

# Test Sightengine credentials from commun/msdev/.env (no secret output)
$ErrorActionPreference = 'Stop'
$root = Split-Path -Parent $PSScriptRoot
$envFile = Join-Path $root 'msdev\.env'
if (-not (Test-Path $envFile)) { throw 'commun/msdev/.env missing' }
...
if (-not $user -or -not $secret) { throw 'SIGHTENGINE credentials missing in commun/msdev/.env' }

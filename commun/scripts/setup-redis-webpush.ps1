# commun/scripts/setup-redis-webpush.ps1 - Redis (VPS or msdev) + VAPID keys for msdev
# Usage:
#   powershell -ExecutionPolicy Bypass -File commun/scripts/setup-redis-webpush.ps1 -Staging
#   powershell -ExecutionPolicy Bypass -File commun/scripts/setup-redis-webpush.ps1 -Prod
#   powershell -ExecutionPolicy Bypass -File commun/scripts/setup-redis-webpush.ps1 -Msdev
param(
    [switch]$Staging,
    [switch]$Prod,
    [switch]$Msdev,
...
function New-VapidKeys {
    Push-Location (Join-Path $root 'commun/backend')
...
function Setup-VpsRedis([string]$sshHost, [string]$pm2App) {
    ...
    $remote = @"
set -e
bash /opt/onscen/deploy/setup-redis-vps.sh
...
function Setup-MsdevEnv {
    Write-Host '>> commun/msdev/.env - VAPID + optional REDIS_URL' -ForegroundColor Cyan
    $envFile = Join-Path $root 'commun\msdev\.env'
    $example = Join-Path $root 'commun\msdev\.env.example'

    if (-not (Test-Path $envFile)) {
        if (Test-Path $example) {
            Copy-Item $example $envFile
            Write-Host '  Copied commun/msdev/.env.example to commun/msdev/.env' -ForegroundColor Yellow
        } else {
            throw 'commun/msdev/.env missing'
        }
    }
    ...
    if ($needsVapid -or $GenerateVapidOnly) {
        ...
        Write-Host '  [OK] VAPID already in commun/msdev/.env' -ForegroundColor Green
    }
...
if ($Staging) { Setup-VpsRedis 'onscen-staging' 'onscen-backend-staging' }
if ($Prod) { Setup-VpsRedis 'onscen-prod' 'onscen-backend' }

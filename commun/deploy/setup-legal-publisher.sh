#!/usr/bin/env bash
# setup-legal-publisher.sh — Crée /opt/onscen/legal-publisher.json si absent
# Usage : bash /opt/onscen/deploy/setup-legal-publisher.sh
#         bash /opt/onscen/deploy/setup-legal-publisher.sh --force  # recopie depuis l'exemple
set -euo pipefail
...
if [[ ${#MISSING[@]} -gt 0 ]]; then
  ...
  echo "  Compléter avant mise en prod, puis : pm2 reload onscen-backend --update-env"
  exit 2
fi

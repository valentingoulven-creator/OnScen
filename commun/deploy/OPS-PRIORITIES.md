# Priorités ops infra — Soundy

> Checklist actionnable post-audit (juillet 2026).  
> **Chemin VPS canonique :** `/opt/soundly` (résolution legacy via `commun/deploy/lib/soundy-root.sh`).
...
| **5** | Nettoyage secrets + doc chemins | 1 h | 0 € | ✅ **Fait** — doc `/opt/soundly` |
...
```bash
node commun/scripts/audit-external-env.cjs /opt/soundly/.env
...
sudo bash /opt/soundly/deploy/install-uploads-backup-cron.sh
sudo bash /opt/soundly/deploy/install-offsite-backup-cron.sh
sudo bash /opt/soundly/deploy/install-pg-client-staging.sh   # une fois
```
...
Instance `soundy-db` : `backup_schedule.disabled=false`, rétention **7 j**, prochain run planifié.  
...
Actions console [Managed Databases → soundy-db → Backups](https://console.scaleway.com/databases) :

- [ ] Sauvegardes automatiques **activées**
- [ ] Dernière backup **< 24 h**
- [ ] Rétention notée (typ. 7 j DB-DEV-S)
- [ ] **Test restore trimestriel** sur instance `soundy_restore_test` (procédure `RUNBOOK-PROD.md`)
...
# Puis sur VPS :
ssh soundy-prod "nano /opt/soundly/.env"   # S3_BUCKET, clés, S3_PUBLIC_BASE_URL
ssh soundy-prod "pm2 reload melosong-backend --update-env"
```
...
```bash
commun/scripts/audit-external-env.cjs /opt/soundly/.env
```

### 5b — Documentation chemins VPS

**Canonique :** `/opt/soundly` · scripts : `/opt/soundly/deploy/` (déployé depuis `commun/deploy/`).

Fichiers corrigés : `INFRA-SOUNDY.md`, `RUNBOOK-PROD.md`, `ENVIRONNEMENTS.md`, scripts deploy.
...
```bash
ssh soundy-prod
source /opt/soundly/deploy/lib/soundy-root.sh
bash "$DEPLOY_DIR/verify-prod.sh"
bash "$DEPLOY_DIR/verify-scaleway-backup.sh"
node /opt/soundly/commun/scripts/audit-external-env.cjs /opt/soundly/.env
```
...
- [`docs/INFRA-SOUNDY.md`](../docs/INFRA-SOUNDY.md)

...
#
# Usage : bash /opt/onscen/deploy/verify-scaleway-backup.sh
set -euo pipefail

echo "=== Checklist sauvegardes Scaleway (manuelle) ==="
echo ""
echo "Managed Database (onscen-prod) — https://console.scaleway.com/databases"
...
echo "  [ ] Snapshot manuel avant chaque upgrade majeur (commun/deploy/deploy_zero_downtime.ps1)"
...
echo "  [ ] Bucket créé (ex. onscen-backups, région fr-par)"
echo "  [ ] Clés API créées → SCW_ACCESS_KEY / SCW_SECRET_KEY dans /opt/onscen/.env"
echo "  [ ] SCW_BUCKET défini → backup-offsite.sh sync S3"
echo ""
echo "VPS local (automatisé via cron) :"
echo "  bash /opt/onscen/deploy/verify-prod.sh"
echo ""
echo "Pour activer les crons locaux :"
echo "  sudo bash /opt/onscen/deploy/install-backup-cron.sh"
echo "  sudo bash /opt/onscen/deploy/install-uploads-backup-cron.sh"
echo "  sudo bash /opt/onscen/deploy/install-offsite-backup-cron.sh"
echo ""
echo "Référence : docs/INFRA-ONSCEN.md · commun/deploy/RUNBOOK-PROD.md"
echo ""

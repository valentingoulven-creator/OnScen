...
if [[ ! -f "$ENV_FILE" ]]; then
  ...
  if grep -qE '^[[:space:]]*DATABASE_URL=.+[^[:space:]]' "$ENV_FILE" 2>/dev/null; then
    ...
    warn "DATABASE_URL absent — repli store.json local (voir commun/deploy/RUNBOOK-PROD.md)"
  fi
...
if [[ -d "$UPLOADS_BACKUP_DIR" ]]; then
  ...
  if [[ "${UP_COUNT:-0}" -eq 0 ]]; then
    warn "Aucune archive uploads — lancer commun/deploy/backup-uploads.sh ou install-uploads-backup-cron.sh"
  else
...
if [[ -d "$OFFSITE_DIR/db" ]]; then
  ...
  if [[ "${OFF_COUNT:-0}" -eq 0 ]]; then
    warn "Copie off-site vide — lancer commun/deploy/backup-offsite.sh ou install-offsite-backup-cron.sh"
  fi
else
...
if command -v crontab >/dev/null 2>&1; then
  ...
  if echo "$CRON" | grep -q 'backup-db.sh'; then
    ...
    warn "Cron backup-db absent — sudo bash commun/deploy/install-backup-cron.sh"
  fi
  if echo "$CRON" | grep -q 'backup-uploads.sh'; then
    ok "Cron backup-uploads installé"
  else
    warn "Cron backup-uploads absent — sudo bash commun/deploy/install-uploads-backup-cron.sh"
  fi
  if echo "$CRON" | grep -q 'backup-offsite.sh'; then
    ok "Cron backup-offsite installé"
  else
    warn "Cron backup-offsite absent — sudo bash commun/deploy/install-offsite-backup-cron.sh"
  fi
fi
...
if [[ "$FAIL" -eq 0 ]]; then
  ...
  echo "Résultat : ÉCHEC — corriger les points ✗ ci-dessus (commun/deploy/RUNBOOK-PROD.md)"
  exit 1
fi

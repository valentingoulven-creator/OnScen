# Mémo R&D — OnScen (CIR / JEI)

**Document interne** · à transmettre à l'expert-comptable / conseil fiscal  
**Date :** 9 juillet 2026  
**Éditeur visé :** SASU OnScen (à constituer) — activité : plateforme sociale musicale getsoundy.com  
**Rédaction :** documentation technique issue du monorepo `C:\Dev\OnScen`  
...
Ce mémo recense les **travaux de recherche et développement (R&D)** menés sur OnScen, distingue les activités **éligibles / non éligibles** au sens du **Crédit d'Impôt Recherche (CIR)** et du statut **Jeune Entreprise Innovante (JEI)**, et propose une **estimation du ratio dépenses R&D / charges déductibles**.
...
**OnScen** est une plateforme sociale **musicale et artistique** (PWA + apps Capacitor) permettant :
...
| Critère JEI | OnScen (aujourd'hui) | Commentaire |
|-------------|----------------------|-------------|
...
| **Travaux OnScen** | Événements Socket.io `sync_playback`, `salon_force_sync` ; gestion drift ; file d'attente salon ; rate limiting ; état playback partagé. |
...
| **Travaux OnScen** | Haversine + blur (`geo.ts`), fallback RAM / PostGIS (`nearbyPeople.ts`, `routes/geo.ts`), clusters grandes villes (`mapMajorCityLiveClusters.ts`), bascule carte 2D / globe 3D (`MapView.tsx`, `GlobeView.tsx`). |
...
| **Travaux OnScen** | `livekit.ts`, `cloudflareStream.ts`, `livekitEgressStore.ts`, modes stream, preview PiP, playback HLS client. |
...
| **Travaux OnScen** | Intégration ACRCloud HMAC, échantillonnage, seuils score, politique fail-closed prod (`acrCloud.ts`, `acrCloudConfig.ts`). |
...
| **État de l'art** | TikTok/Instagram : modèles propriétaires ; pas applicable directement au graphe social limité OnScen. |
| **Travaux OnScen** | Pondération configurable, algo intégré OnScen (`reelFeedRanking.ts`), affinités musicales, feed actualités. |
...
| **Travaux OnScen** | Intégration Sightengine, workflows modération admin, blocage publication. |
...
| **Travaux OnScen** | `pgStore.ts`, `pgSalonsLives.ts`, `pgReels.ts`, mutex persistance, plan `STACK-CIBLE.md`. |
...
### 6.3 Scénario recommandé pour OnScen (2026–2027)
...
*Document généré pour OnScen / getsoundy.com — juillet 2026. À mettre à jour à chaque clôture comptable ou pivot technique majeur.*

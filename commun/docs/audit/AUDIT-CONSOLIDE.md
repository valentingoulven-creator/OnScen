# AUDIT CONSOLIDÉ — Soundy (getsoundy.com)
...
| `pm2 jlist` sur `soundy-prod` | `melosong-backend` : **`instances:1`, `exec_mode:cluster_mode`, uptime 2h** au moment du contrôle — **la mitigation `instances:1` est maintenant réellement déployée en prod** (elle ne l'était pas au 08/07). |
...
| `psql` prod — privilèges du rôle `soundy` | `rolcreaterole=t, rolcreatedb=t` — **toujours sur-privilégié**, non traité. |
...
| DB staging (`soundy-staging`) | `DATABASE_URL` pointe toujours vers le même hôte `51.15.132.229:14440` que la prod (base logique différente `soundy_staging`) — **toujours partagé**. |
...
- `APP_ENV` effectif du process `melosong-backend` (`pm2_env.env.APP_ENV`) : `production` — confirmé, `NODE_ENV=production` également. `DONATIONS_ENABLED=1`, `SUBSCRIPTIONS_ENABLED=0` (les abonnements créateurs sont donc actuellement désactivés en prod, seuls les dons/pourboires live sont concernés par ce risque).
- **Tous** les 9 fichiers `.env.bak*` présents sur `/opt/soundly/` (`.env.bak.`, `.env.bak.manual`, `.env.bak-resend-*`, `.env.bak.202606*` × 4, `.env.bak.livekit`) ont été grep-és pour `STRIPE_SECRET_KEY`/`STRIPE_PUBLISHABLE_KEY` : **100% en `sk_test_`/`pk_test_`**, aucun historique de clé live sur ce VPS.
...
| 5 | **Révoquer `CREATEROLE`/`CREATEDB` du rôle DB `soundy`** en prod (`REVOKE CREATEROLE, CREATEDB FROM soundy;`) | DB-Infra | S | Confirmé toujours actif aujourd'hui ; privilège excessif pour un compte applicatif, action simple une fois validée |

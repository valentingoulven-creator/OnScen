...
>
> **🔄 Rafraîchissement 2026-08-11 (soir)** : vérifié directement sur le VPS prod — `dist/lib/ageGates.js` déployé contient bien `MIN_LIVE_AGE = 16` et la logique `GEO_PRECISE_MIN_AGE`. **La restriction géo mineurs confirmés est donc désormais active en production**, pas seulement dans le code source.
...
| Mineurs | ✅ **Implémenté depuis le 08-10, déployé en prod le 08-11 (soir)** (`ageGates.GEO_PRECISE_MIN_AGE = 18`, `locationPrivacy.enforceMinorGeoPolicy` / `userRequiresCityOnlyGeo`) : les comptes < 18 ans **confirmés** sont forcés en précision « ville » (`locationPrecision = 'city'`), sans GPS live, appliqué à l'inscription (`auth.ts`/`oauth.ts`) et au changement de préférences (`applyPrivacySettings`). Vérifié présent dans `dist/lib/ageGates.js` sur le VPS prod | résolu (code + déployé) | Ajouter un test E2E « mineur ne peut pas activer géo précise » ; surveiller le volume de nouveaux comptes mineurs confirmés maintenant que les inscriptions prod sont **ouvertes** (`ACCESS_REGISTRATION_MODE=open`, cf. [06-ddos §6.3](./06-ddos.md)) |
...
**Mise à jour 2026-08-11 :** la restriction géo mineurs (E2 de la synthèse) est **implémentée** dans le code. Le risque de régression sur les comptes « âge inconnu » (quantifié à 95 % des comptes actifs) a été **corrigé** avant tout déploiement — voir §3.2.

**Mise à jour 2026-08-11 (soir) :** **déploiement confirmé en production** (vérifié directement sur le build `dist/` du VPS). E2 peut être considéré **totalement résolu** (code + prod), sous réserve du point produit ouvert sur le seuil live 16 ans (cf. [11-legalite-globale](./11-legalite-globale.md)).

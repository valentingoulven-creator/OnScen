...
| ✅ Process `soundy-auth` fantôme | **Résolu 2026-08-11** : identifié comme résidu de la migration de domaine — gate basic-auth minimal (`/opt/onscen/deploy/auth-server/server.js`, un seul compte `dev` avec hash bcrypt codé en dur). Vérifié non routé par les 3 configs Caddy actives (0 référence) et écoutant uniquement sur `127.0.0.1:3001` (non exposé publiquement). Supprimé du process manager (`pm2 delete soundy-auth && pm2 save`). `onscen-backend` non affecté, `/health` OK après suppression | résolu | — |

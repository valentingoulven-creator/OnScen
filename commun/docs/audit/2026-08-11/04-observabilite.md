...
| Mobile Capacitor | ✅ **Résolu depuis le 08-10** : `@sentry/react` `^10.70.0` ajouté aux dépendances `ios/apptel` ; `initNativeSentry()` (`lib/sentryNative.ts`) appelé dans `main.tsx`, DSN via `VITE_SENTRY_DSN`, désactivé en `msdev`/`DEV`, `sendDefaultPii: false` | résolu (code) | Confirmer build store réel avec DSN mobile configuré + vérifier remontée d'un crash test avant publication |
...
**Mise à jour 2026-08-11 :** Sentry mobile (E8) est **implémenté** côté code (non déployé en build store). Restent à combler : **monitoring infra hors Sentry**, **logs structurés redactés**.

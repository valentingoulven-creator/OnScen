...
3. VPS `/opt/soundy/.env` : `SCW_BUCKET`, `SCW_REGION=fr-par`, `SCW_ACCESS_KEY`, `SCW_SECRET_KEY` (voir `commun/deploy/.env.production.example`).
4. VPS : `bash /opt/soundy/commun/deploy/setup-scaleway-object-storage.sh --vps-only` (installe `awscli`, teste `backup-offsite.sh`).
...
```bash
# Vérifier un dump
bash /opt/soundy/commun/deploy/verify-backup.sh

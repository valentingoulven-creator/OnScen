...
| **Validité** | [DATE_VALIDITE] *(30 jours calendaires recommandés)* |
| **Référence campagne** | [REF_CAMPAGNE] |
...
| Champ | Valeur |
|-------|--------|
| **Raison sociale** | [ANNONCEUR] |
| **Forme juridique** | [FORME_JURIDIQUE_ANNONCEUR] |
| **SIRET** | [SIRET] |
...
| **Durée** | [NOMBRE_SEMAINES] semaine(s) *(unité de facturation : la semaine, par emplacement ou par pack)* |
| **Zone géographique** | [ ] Hyper-local (rayon [RAYON_KM] km autour de [VILLE]) · [ ] Ville : [VILLE] · [ ] Région : [REGION] · [ ] France entière |
| **Lien CTA** | [URL_CTA] *(billetterie, site, landing — HTTPS obligatoire)* |

---

## Catalogue emplacements (forfait / semaine · sans CPM)

Chaque ligne correspond à **un emplacement dédié** : une campagne = un format, une créa adaptée, une diffusion sur toute la semaine réservée. **Aucun calcul au nombre de vues.**
...
## Détail du devis (lignes campagne)
...
## Offres multi-emplacements (packs hebdomadaires)

Alternative au choix ligne à ligne : **forfait pack** pour une même semaine (ou [NOMBRE_SEMAINES] semaines × prix pack).
...
## Conditions commerciales (indicatives)
...
## Annexe créative — specs visuelles (résumé)

| Emplacement | Logo | Bannière / vidéo |
|-------------|------|------------------|
| Fil d'actualité (`feed_inline`) | 48×48 px | 343×120 px |
| Carte · bandeau (`map_banner`) | 80×80 px | 360×90 px min. |
| Carte · icône Sponso (`map_sidebar_events`) | 80×80 px | Affiche événement + titre |
| Reel sponsorisé (`reels_sponsored`) | 64×64 px | 1080×1920 px · MP4 ≤ 30 s recommandé |
| Onglet Musique (`music_tab`) | 48×48 px | 343×120 px *(ou specs communiquées à la commande)* |

---

## Acceptation du devis

Le présent devis vaut **proposition commerciale** jusqu'à la date de validité. La campagne démarre après :

1. Acceptation écrite (email ou signature ci-dessous)
2. Réception de l'acompte
3. Validation des assets créatifs

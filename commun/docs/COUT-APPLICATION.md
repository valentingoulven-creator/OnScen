...
|--------|-------------|------|
| Frontend | React + Vite (`app/`, `ios/apptel/`) | UI web et PWA mobile |
...
| Process manager | PM2 (`commun/deploy/ecosystem.config.cjs`) | Autorestart, logs, zero-downtime reload |
| Reverse proxy | Caddy (`commun/deploy/Caddyfile`) | HTTPS getsoundy.com, Basic Auth UI |
| WebRTC relay | Coturn (même VPS) | TURN pour mesh WebRTC derrière NAT |
| Déploiement | `commun/scripts/deploy-prod.ps1` → `commun/deploy/deploy_zero_downtime.ps1` | Build + sync VPS + `pm2 reload` |
...
| Lancement | `npm run dev` | `commun/scripts/deploy-prod.ps1` |

Références : [`docs/DEV-WORKFLOW.md`](DEV-WORKFLOW.md), [`commun/deploy/RUNBOOK-PROD.md`](../commun/deploy/RUNBOOK-PROD.md).
...
> Les sauvegardes PostgreSQL Scaleway (automatiques console) et les dumps VPS (`commun/deploy/backup-db.sh`, cron 03:15) sont inclus dans les plans respectifs sans surcoût significatif au stade MVP.
...
> **PostgreSQL :** au stade Scale, envisager **DB-PRD-S (~50 €/mois)** et `PG_POOL_MAX=20` (voir `commun/deploy/README.md`).
...
| Setup PostgreSQL | [`commun/deploy/README.md`](../commun/deploy/README.md) |

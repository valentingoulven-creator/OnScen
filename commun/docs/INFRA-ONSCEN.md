...
| Chemin app | `/opt/onscen` | `/opt/onscen` |
| PostgreSQL | `onscen-prod` | `onscen_staging` (même instance `51.15.132.229:14440`) |
| PM2 | `onscen-backend` | `onscen-backend-staging` |
...
| Uploads utilisateur | Variable VPS | `/opt/onscen/public/uploads/` — **backup hebdo** (`backup-uploads.sh`) |

---

## Sauvegardes — triple couche

### Couche 1 — VPS DB (`commun/deploy/backup-db.sh`)

| Paramètre | Valeur |
|-----------|--------|
| Fréquence | Quotidien **03:15** (cron) |
| Rétention | **14 jours** (`RETENTION_DAYS=14`) |
| Sortie | `/opt/onscen/backups/onscen-YYYYMMDD-HHMMSS.sql.gz` |
| **RPO** | **≤ 24 h** (pire cas juste après backup) |
| **RTO** | **30 min – 2 h** (restore sur base test) |

### Couche 1b — VPS uploads (`commun/deploy/backup-uploads.sh`)

| Paramètre | Valeur |
|-----------|--------|
| Fréquence | Hebdomadaire **dim. 04:30** (cron) |
| Rétention | **28 jours** |
| Sortie | `/opt/onscen/backups/uploads/uploads-YYYYMMDD-HHMMSS.tar.gz` |
| **RPO uploads** | **≤ 7 j** |

### Couche 1c — Copie off-site VPS (`commun/deploy/backup-offsite.sh`)

| Paramètre | Valeur |
|-----------|--------|
| Fréquence | Quotidien **04:00** (après pg_dump) |
| Destination locale | `/opt/onscen/backups-offsite/` |
...
3. VPS `/opt/onscen/.env` : `SCW_BUCKET`, `SCW_REGION=fr-par`, `SCW_ACCESS_KEY`, `SCW_SECRET_KEY` (voir `commun/deploy/.env.production.example`).
4. VPS : `bash /opt/onscen/commun/deploy/setup-scaleway-object-storage.sh --vps-only` (installe `awscli`, teste `backup-offsite.sh`).
5. `mkdir -p /opt/onscen/public/uploads` ; `backup-uploads.sh` archive dès qu’il y a des fichiers.

Helper : `commun/deploy/setup-scaleway-object-storage.sh` (création bucket via `scw` si CLI configuré).
...
```bash
# Vérifier un dump
bash /opt/onscen/commun/deploy/verify-backup.sh

# Restore test (NE PAS sur prod sans maintenance)
gunzip -c /opt/onscen/backups/onscen-XXXX.sql.gz | psql "$DATABASE_URL"
```
...
| Lancement | `npm run dev` | `commun/deploy/deploy_zero_downtime.ps1` |
| Bots démo | Oui | Non |
...
| commun/msdev/store.json + iCloud | Moyenne | Ne pas sync iCloud |
...
| Script | Rôle |
|--------|------|
| `commun/deploy/backup-db.sh` | Dump PostgreSQL → backups/ |
| `commun/deploy/backup-uploads.sh` | Archive tar.gz uploads utilisateur |
| `commun/deploy/backup-offsite.sh` | Copie secondaire DB + uploads (+ S3 optionnel) |
| `commun/deploy/install-uploads-backup-cron.sh` | Cron hebdo uploads (dim. 04:30) |
| `commun/deploy/install-offsite-backup-cron.sh` | Cron quotidien off-site (04:00) |
| `commun/deploy/setup-scaleway-object-storage.sh` | Bucket + awscli VPS + test off-site |
| `commun/deploy/verify-scaleway-backup.sh` | Checklist manuelle console Scaleway |
| `commun/deploy/snapshot-vps-reminder.sh` | Rappel snapshot VPS avant upgrade |
| `commun/deploy/verify-backup.sh` | Intégrité dump `.sql.gz` |
| `commun/deploy/verify-prod.sh` | Checklist ops VPS (+ âge backups) |
| `commun/deploy/install-backup-cron.sh` | Cron 03:15 |
| `commun/deploy/ecosystem.config.cjs` | PM2 (512M, autorestart) |
| `commun/deploy/healthcheck.sh` | Redémarre PM2 si /health KO |
...
- [`commun/deploy/README.md`](../commun/deploy/README.md)
- [`commun/deploy/RUNBOOK-PROD.md`](../commun/deploy/RUNBOOK-PROD.md)

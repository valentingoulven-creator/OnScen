...
flowchart LR
  ...
  subgraph DEV["MODE DEV — localhost"]
    ...
    Data["commun/msdev/data/store.json"]
  ...
  DEV -.->|"deploy prod uniquement\ncommun/scripts/deploy-prod.ps1"| PROD
...
| Deploy | — | `commun/scripts/deploy-prod.ps1` |
...
powershell -ExecutionPolicy Bypass -File commun/scripts/dev-start.ps1

powershell -ExecutionPolicy Bypass -File commun/msdev/LANCER-DEV.ps1
...
3. Appelle `commun/deploy/deploy_zero_downtime.ps1 -VerifyProd`
...
powershell -ExecutionPolicy Bypass -File commun/scripts/deploy-prod.ps1 -SkipBuild

powershell -ExecutionPolicy Bypass -File commun/scripts/deploy-prod.ps1 -SkipFrontend



# Demander confirmation si le dépôt n'est pas propre

powershell -ExecutionPolicy Bypass -File commun/scripts/deploy-prod.ps1 -AskCommit
...
5. **Production** — `commun/scripts/deploy-prod.ps1` ou **« deploy prod »** à l'agent (règle `.cursor/rules/deploy-prod.mdc`).
...
Voir [`commun/deploy/RUNBOOK-PROD.md`](../commun/deploy/RUNBOOK-PROD.md). Résumé :
...
- Deploy scripts : [`commun/deploy/README.md`](../commun/deploy/README.md)

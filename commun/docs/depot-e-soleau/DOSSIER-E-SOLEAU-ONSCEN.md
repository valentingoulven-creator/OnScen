...
| Champ | Valeur |
|-------|--------|
| **Titre du document** | Description intégrale de l’application OnScen — réseau social musical |
| **Nom commercial principal** | **OnScen** |
...
Ce document décrit fidèlement l’état du produit OnScen tel que documenté dans le dépôt source au 29 juin 2026. Il constitue une **preuve descriptive** de l’existence, du contenu et de l’architecture de l’application à la date indiquée.
...
| Nom | Usage | Contexte |
|-----|-------|----------|
| **OnScen** | Marque publique principale | Interface utilisateur, CGU, domaine getsoundy.com, stores mobiles (`appName: "OnScen"`) |
| **OnScen+** | Abonnement premium plateforme | Réduction publicités, filtres carte, quotas live étendus |
| **OnScenUltra** | Abonnement premium avancé | Diffusion OBS / Cloudflare Stream, quotas live élevés |
...
- **Identité graphique :** dégradé « Wave OnScen » appliqué aux pseudonymes et éléments de navigation
...
| Élément | Valeur |
|---------|--------|
| Éditeur | Valentin Goulven (OnScen) |
...
OnScen est une **plateforme sociale musicale** permettant notamment :
...
- de publier ou consulter des reels, de suivre des utilisateurs, d’envoyer des réactions, des pourboires en live ou des abonnements créateurs / OnScen+.

**Le Service n’est pas un service de rencontre.**

### 2.2 Mission produit

OnScen vise à **unifier** ce que les fans de musique dispersent aujourd’hui entre plusieurs applications :

| Besoin utilisateur | Applications actuelles | Réponse OnScen |
|--------------------|------------------------|----------------|
| Découverte musicale | TikTok, Instagram Reels | Reels, feed actualités, algo OnScen |
...
- **Publication stores :** dépôt prévu sur l’**App Store Apple (iOS)** et **Google Play (Android)** sous le nom **OnScen**, identifiant `com.soundy.app`
...
- Notation hôte, archivage rediffusions (OnScen+ / OnScenUltra)
...
**Description :** Carte interactive centrale de l’expérience OnScen.

**Éléments affichés :**
- Salons d’écoute actifs
- Lives en cours
- Événements musicaux (~30 km)
- Personnes proches (comptes OnScen avec géolocalisation activée)
...
- Configuration plans plateforme (OnScen+, OnScenUltra)
...
OnScen intègre un **réseau publicitaire natif** directement dans l’expérience musicale. Le sponsoring est la **priorité court terme** du modèle économique (45–55 % du revenu cible à M24).
...
- Transparence : badge « Sponsorisé » vs « Promo » (interne OnScen)
...
| **OnScen+ / OnScenUltra** | Abonnement plateforme 2,99–4,99 €/mois | 10–15 % |
| **B2B lieux** | OnScen Pro Lieu 49–149 €/mois (roadmap) | 5–15 % |
...
| OnScen+ | `soundy_plus` | ~2,99–4,99 €/mois | Moins de sponsors, filtres carte, quotas étendus |
| OnScenUltra | `soundy_ultra` | Premium | OBS/Cloudflare Stream, quotas live élevés, rediffusions |
...
1. **Salons YouTube synchronisés** — écoute partagée via le lecteur officiel YouTube, sans re-streaming audio centralisé par OnScen
...
OnScen est conçu pour un **public mondial** : fans de musique, créateurs, lieux et marques, où que se trouvent leurs communautés.
...
|--------|-------------------|-------|
| **OnScen** | 9 (logiciel), 38 ( télécoms), 41 (divertissement), 42 (SaaS) | Marque principale |
| **OnScen+** | 9, 41 | Abonnement premium |
| **OnScenUltra** | 9, 41 | Abonnement premium avancé |
...
| PostgreSQL | Instance managée Scaleway (onscen-prod) |
...
| Base | onscen_staging (même instance PG) |
...
├── lib/
│   ├── platformPlans.ts      → Plans OnScen+/Ultra
...
Je soussigné(e), **Valentin Goulven**, déclare être l’auteur de l’application **OnScen** et de la documentation qui en décrit l’architecture, les fonctionnalités et le modèle économique, tels que présentés dans ce document à la date du **29 juin 2026**.

Ce document a été établi à partir du dépôt source OnScen situé au chemin local `C:\Dev\OnScen` et reflète l’état du produit accessible publiquement sur **https://getsoundy.com**.

---

**FIN DU DOCUMENT**

*Document généré pour dépôt e-Soleau INPI — OnScen — 29 juin 2026*

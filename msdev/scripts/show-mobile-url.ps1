# URL MeloSong pour smartphone (lit msdev/.env + compare a l'IP reelle)
...
if ($detected -and $detected -notcontains $fixedIp) {
    ...
    Write-Host "  Mettez a jour msdev/.env (MOBILE_HOST_IP) et MOBILE-URL.txt" -ForegroundColor Yellow
}
Write-Host ""

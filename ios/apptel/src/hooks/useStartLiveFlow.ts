/**
 * Fichier override mobile — ios/apptel/src/hooks\useStartLiveFlow.ts
 * Créé via: npm run mobile:override -- create hooks\useStartLiveFlow.ts
 * Ne pas modifier web/app/src/hooks\useStartLiveFlow.ts pour ce comportement mobile-only.
 */
...
export function useStartLiveFlow({
  ...
  const startLive = useCallback(() => {
    ...
    // Tel / IAP : live sans Connect par défaut (pas de Stripe dans la WebView).
    setLiveTipsSkipped(true);
    setStripeConnectSkipped();
    setMediaSetupOpen(true);

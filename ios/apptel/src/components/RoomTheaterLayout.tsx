/**
 * Fichier override mobile — ios/apptel/src/components\RoomTheaterLayout.tsx
 * Créé via: npm run mobile:override -- create components\RoomTheaterLayout.tsx
 * Ne pas modifier web/app/src/components\RoomTheaterLayout.tsx pour ce comportement mobile-only.
 */
import './RoomTheaterLayout.mobile.css';
...
import { TikTokLiveChatOverlay } from './TikTokLiveChatOverlay';
import { LiveVideoTapToPause } from './LiveVideoTapToPause';
import { LiveLandscapeExpand } from './LiveLandscapeExpand';
...
const DOCK_HEADER_ICON_BTN_COMPACT =
  'shrink-0 w-8 h-8 flex items-center justify-center rounded transition touch-manipulation';
...
function DockedChatHeader({
  ...
  const iconBtn = compact ? DOCK_HEADER_ICON_BTN_COMPACT : DOCK_HEADER_ICON_BTN;
        ...
        <div className="flex items-center gap-1.5 px-3 pt-2 pb-1.5">
          ...
          {onTogglePin ? (
            <button
              type="button"
              onClick={onTogglePin}
              title={pinned ? 'Détacher le chat' : 'Épingler en bas'}
              className={`${iconBtn} ${
                pinned
                  ? 'text-amber-300 bg-amber-950/40'
                  : 'text-gray-500 hover:text-amber-300 hover:bg-white/10'
              }`}
              aria-label={pinned ? 'Détacher le chat' : 'Épingler le chat en bas'}
              aria-pressed={pinned}
            >
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden>
                <path
          ...
          <button
            type="button"
            onClick={onToggleChat}
            className={`${iconBtn} text-gray-500 hover:text-white hover:bg-white/10 text-lg leading-none`}
            aria-label="Masquer le chat"
          >
            ×
          </button>
  ...
  return (
    <div
      className={`shrink-0 flex items-center min-w-0 overflow-hidden border-b border-[#1e1e2f] bg-[#14141c]/80 ${
        compact
          ? 'room-theater-dock-header--compact gap-1 px-2 py-0.5'
          : 'gap-1.5 px-3 py-2'
      }`}
    >
...
export interface RoomTheaterLayoutProps {
  ...
  /** Live : chat épinglé en bas (vs flottant). */
  chatPinned?: boolean;
...
export function RoomTheaterLayout({
  ...
  /** Live mobile : chat overlay TikTok (sur la vidéo), pas de dock. */
  const useTikTokLiveChat = liveTheaterChrome;
  const pinLiveChatBottom = false;
  ...
  const showLeftDock = !useTikTokLiveChat && !chatHidden && theaterDock === 'left';
  const showRightDock = !useTikTokLiveChat && !chatHidden && theaterDock === 'right';
  const showFloating = !useTikTokLiveChat && !chatHidden && theaterDock === 'floating';
  const showBottomDock = !useTikTokLiveChat && !chatHidden && theaterDock === 'bottom';
  ...
  const useVideoStack = (stackBelowVideo || pinLiveChatBottom) && theaterDock === 'bottom';
  ...
  /** Live TikTok : pas de bouton 💬 — le chat est toujours en overlay. */
  ...
  const tiktokChatWindow =
    useTikTokLiveChat && !chatHidden && !chatMinimized ? (
      <TikTokLiveChatOverlay chat={chat} chatInput={chatInput} />
    ) : null;
  ...
  const overlayChatWindow = tiktokChatWindow ?? floatingChatWindow;
  const mountChatInVideoContainer = liveTheaterChrome && !!overlayChatWindow;
  const stageCloneProps: Record<string, unknown> = {};
  if (mountChatInVideoContainer && overlayChatWindow) {
    stageCloneProps.floatingChat = overlayChatWindow;
  }
  ...
  const videoStage = (
    <div
      className={`room-theater-video-stage relative flex flex-col ${
        pinLiveChatBottom || !useVideoStack ? 'flex-1' : 'shrink-0'
    ...
    >
      ...
      {!mountChatInVideoContainer && overlayChatWindow}

      {chatHiddenButton}
      {useTikTokLiveChat ? <LiveVideoTapToPause /> : null}
      {liveTheaterChrome ? <LiveLandscapeExpand /> : null}
    </div>
  );

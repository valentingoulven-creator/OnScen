/**
 * Fichier override mobile — ios/apptel/src/components\MapCreateActionFab.tsx
 * Créé via: npm run mobile:override -- create components\MapCreateActionFab.tsx
 * Ne pas modifier web/app/src/components\MapCreateActionFab.tsx pour ce comportement mobile-only.
 */
...
import { useMapCreateIntent } from '../hooks/useMapCreateIntent';
...
export function MapCreateActionFab({
  ...
  const { startCreate, handoff, handoffLoading, handoffError, confirmHandoff, cancelHandoff } =
    useMapCreateIntent({ onStartLive, onCreateEvent, onCreateSalon });
  ...
  const choose = (kind: 'live' | 'event' | 'salon') => {
    setOpen(false);
    startCreate(kind);
  };
  ...
  return (
    ...
    >
      ...
      <ConfirmModal
        ...
        confirmLabel={handoff?.confirmLabel ?? 'Continuer'}
        cancelLabel="Annuler"
        destructive={false}

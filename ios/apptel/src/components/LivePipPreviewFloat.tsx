...
import { getLivePipPosition, setLivePipPosition } from '../lib/livePipPosition';
...
function LivePipPreviewFloatInner({
  ...
  const { token } = useAuth();
  // Mobile only : pas de resetKey (live.id) — le PiP garde la dernière position
  // laissée par l'utilisateur, même en changeant de live (cf. livePipPosition.ts).
  const pip = useDraggableVideoPip(true, onClose, () => getLivePipPosition(defaultVideoPipPos));
  const videoH = Math.round((VIDEO_PIP_WIDTH * 9) / 16);
  const [hostFollowing, setHostFollowing] = useState(false);

  useEffect(() => {
    setLivePipPosition(pip.position);
  }, [pip.position]);

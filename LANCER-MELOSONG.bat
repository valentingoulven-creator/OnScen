...
if not errorlevel 1 (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0msdev\scripts\resolve-msdev-url.ps1" -Open
  echo.
...
:: 3. Forcer mode HTTP (ignore MSDEV_HTTPS=1 du fichier msdev/.env)
set MSDEV_HTTPS=0

:: 3b. Fichier backend manquant (doublon iCloud « msdevLanConfig 2.ts »)
if not exist ""backend\src\lib\msdevLanConfig.ts"" (
  if exist ""backend\src\lib\msdevLanConfig 2.ts"" (
    echo  [INFO] Restauration de msdevLanConfig.ts ^(copie iCloud^)...
    copy /y ""backend\src\lib\msdevLanConfig 2.ts"" ""backend\src\lib\msdevLanConfig.ts"" >nul
  )
)
